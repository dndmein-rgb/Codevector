...
async function injectConcurrentWrites(): Promise<void> {
  console.log(
...
async function run(): Promise<void> {
  ...
  const idsBeforeBrowsing: Set<string> = new Set(
    before.map((r: { id: bigint }) => r.id.toString()),
  );
  console.log(
    `Products in DB before browsing starts: ${idsBeforeBrowsing.size}`,
  );
  ...
  console.log(
    `Finished browsing. Pages fetched: ${pageCount}, products seen: ${seenSet.size}`,
  );
  ...
  for (const id of idsBeforeBrowsing) {
    if (!seenSet.has(id)) {
      missed++;
      console.error(
        `  !! MISSED product that existed before browsing: id=${id}`,
      );
    }
  }
  ...
  const sawInjectedDuringBrowse = injectedRow
    ...
    : false;
  console.log(
    `Newly-inserted product appeared in this browse session: ${sawInjectedDuringBrowse} ` +
